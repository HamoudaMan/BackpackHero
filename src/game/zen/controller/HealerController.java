...
public class HealerController {
	...
	public boolean healSmall(Hero hero,HealerState state) {
		if(state.isUsed() || hero.gold() < SMALL_HEAL_COST ) {
			return false;
		}
			hero.spendGold(SMALL_HEAL_AMOUNT);
			hero.heal(SMALL_HEAL_AMOUNT);
	...
	public boolean healFull(Hero hero,HealerState state) {
		if(state.isUsed() || hero.gold() < FULL_HEAL_COST ) {
			return false;
		}
		hero.spendGold(FULL_HEAL_COST);
		hero.healMax();
		state.use();
		return true;
	}
