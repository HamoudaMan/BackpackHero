...
import game.model.dungeon.rooms.CorridorRoom;
import game.model.dungeon.rooms.EnemyRoom;
import game.model.dungeon.rooms.ExitRoom;
import game.model.dungeon.rooms.HealerRoom;
import game.model.dungeon.rooms.MerchantRoom;
import game.model.dungeon.rooms.TreasureRoom;
import game.model.enemy.EnemyType;
import game.model.enemy.EnemyFactory;
...
public final class RandomFloorGenerator {
	 ...
	 * @return Coord  
	 */
	private DCoord chooseHeroStart() {
		return new DCoord(random.nextInt(ROWS), 0);
	}
	
	private boolean validPosition(DCoord c) {
	 ...
	 * @return Floor
	 */
	private List<DCoord> generateMainPath(Room[][] rooms, DCoord heroStart ){
		List<DCoord> mainPath = new ArrayList<>();//array that will store the main path
	 ...
	 * @param rooms
	 */
	private DCoord placeExit(List<DCoord> mainPath,Room[][] rooms ) {
		if(mainPath.isEmpty()) {
		...
		return new DCoord(exitRow, exitCol);
	}
	/**
	...
	private void placeEnemyRooms(List<DCoord> mainPath, Room[][] rooms, DCoord heroStart, DCoord exit) {
		...
		var candidates = new ArrayList<DCoord>(mainPath);
	 ...
	 * @return boolean
	 */
	private boolean isAdjacent(DCoord coord,  Set<DCoord> exsistingRooms) {
		/*
		var dr = Math.abs(a.row() - b.row());
		var dc = Math.abs(a.col() - b.col());
		return dr +dc ==1;*/
		var directions = List.of(new DCoord(-1, 0), new DCoord(1, 0), new DCoord(0, -1), new DCoord(0, 1));//a list of directions (up, down, left, right)
	...
	private boolean forbiddenDiagonal(DCoord coord, Room[][] rooms) {
		var diagonals = List.of(new DCoord(-1, -1), new DCoord(-1, 1), new DCoord(1, -1), new DCoord(1, 1));//a list of diagonals
		for(var d: diagonals) {
			...
			if(rooms[diag.row()][diag.col()] != null) {
				var a = new DCoord(coord.row(), diag.col());
				var b = new DCoord(diag.row(), coord.col());
	...
	private DCoord randomFreeNeighboor(DCoord coord, Room[][] rooms, Set<DCoord> exsistingRooms) {
		var directions = List.of(new DCoord(-1, 0), new DCoord(1, 0), new DCoord(0, -1), new DCoord(0, 1));//a list of directions (up, down, left, right)
		var candidates = new ArrayList<DCoord>();
	...
	private List<DCoord> generateDeadEnds(List<DCoord> mainPath,Room[][] rooms) {
		Objects.requireNonNull(mainPath);
		Objects.requireNonNull(rooms);
		
		var deadEnds = new ArrayList<DCoord>();
...
*/
	...
	private void placeOtherRooms(List<DCoord> mainPath,List<DCoord> deadEnds, Room[][] rooms, DCoord heroStart, DCoord exit) {
		Objects.requireNonNull(mainPath);
